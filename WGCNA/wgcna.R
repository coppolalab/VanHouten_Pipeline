...
PEER.weights <- select(read_csv("../baseline/weight_4.csv"), -(X1:X4))
PEER.factors <- select(read_csv("../baseline/factor_4.csv"), -(X1:X4))
PEER.residuals <- as.matrix(PEER.factors) %*% t(as.matrix(PEER.weights)) %>% t
intensities.PEER <- (intensities1.norm - PEER.residuals) %>% t
...
sft.PEER <- pickSoftThreshold(intensities.PEER, powerVector = powers, verbose = 5, networkType = "signed")
...
gene.module.membership.long <- gather(gene.module.membership, module.comparison, correlation, lightcyan:mediumpurple3)
module.membership.pvalue.long <- gather(module.membership.pvalue, module.comparison, p.value, lightcyan:mediumpurple3)
...
rownames(PEER.factors) <- rownames(intensities.PEER)
colnames(PEER.factors) <- paste("X", 1:ncol(PEER.factors), sep = "")

targets.status <- model.matrix( ~ 0 + factor(targets.final$Status) )[,-1] %>% data.frame
#targets.status <- targets.status[,c(2,1,3)]
colnames(targets.status) <- "Patient"
targets.status %<>% mutate(SampleID = targets.final$SampleID)
